...
export async function createShipment(role: string, userId: string, rawInput: unknown) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.shipmentStatusHistory.create({
      data: { shipmentId: shipment.id, toStatus: "NEW", changedByUserId: userId },
    });
    return shipment;
  });
}

export async function listShipments(role: string, query: {
...
export async function transitionShipment(
  role: string, userId: string, reference: string, toStatus: ShipmentStatus, reason?: string,
) {
  if (!can(role, permissionActions.shipmentUpdateStatus)) throw new Error("FORBIDDEN");
  return prisma.$transaction(async (tx) => {
    const shipment = await tx.shipment.findUnique({
      where: { reference }, select: { id: true, reference: true, status: true },
    });
    ...
    await tx.shipmentStatusHistory.create({
      data: { shipmentId: shipment.id, fromStatus, toStatus, reason: reason?.trim() || undefined, changedByUserId: userId },
    });
    return updated;
  });
}
